...
class FlexJointBase extends BaseSim {
    ...
    systemEquation(t, x, u, z) {
        ...
        x_dot = [
            x[3 - 1] - x[0] * epsilon,
            x[4 - 1],
            1 / J_m * (k * x[2 - 1] - (B_m + K_m ** 2 / R_m) * x[3 - 1] - MC * Math.tanh(1000 * x[3 - 1]) + c * x[4 - 1] + K_m / R_m * u[1 - 1]),
            -(k / J_r + k / J_m) * x[2 - 1] + (B_m + K_m ** 2 / R_m) / J_m * x[3 - 1] + MC * Math.tanh(1000 * x[3 - 1]) / J_m - (c / J_r + c / J_m) * x[4 - 1] - K_m / R_m / J_m * u[1 - 1] + z[0]
        ];
